...
class BrushTicket(object):
    ...
    def __init__(self, passengers, from_time, from_station, to_station, numbers, seat_type,
                 receiver_email):
        """定义实例属性，初始化"""
        # 乘客姓名
    ...
    def send_mail(self, receiver_address, content):
        ...
        host = 'smtp.163.com'
        port = 25
        sender = 'xxxx@163.com'  # 你的发件邮箱号码
        pwd = 'xxxx'  # 不是登陆密码，是客户端授权密码
        # 发件信息
...
if __name__ == '__main__':
    # 乘客姓名
    passengers_input = 'xxx,xxx'
    passengers = passengers_input.split(",")

    from_time = '2019-01-12'#input('请输入乘车日期（例如“2018-08-08”）：')
    ...
    # 出发站
    from_station = '%u90D1%u5DDE%2CZZF'

    # 终点站
    to_station = '%u6DF1%u5733%2CSZQ'
    # 车次编号
    #numbers = ['Z90','Z190','Z14','Z168','Z98','T124','T370']
    numbers = ['G817','G821','G75']

    # 座位类型
    seat_type = '二等座'

    #通知邮件
    receiver_email = 'xxxxxx@163.com'

    #receiver_email = input('请预留一个邮箱，方便抢到票后进行通知（例如：test@163.com）：')
    # while receiver_email == '':
    #     receiver_email = input('预留邮箱不能为空，请重新输入：')
    # 开始抢票
    ticket = BrushTicket(passengers, from_time, from_station, to_station, numbers, seat_type,
                         receiver_email)
    ticket.start_brush()
